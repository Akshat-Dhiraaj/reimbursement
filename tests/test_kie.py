"""Tests for the shared keyword/position KIE — date finding, money parsing, row
reconstruction, and the heuristics that map positioned text lines onto a Receipt. Pure
and model-free: no OCR weights, no PDF opened. Both the docTR OCR extractor and the
pypdfium2 PDF extractor feed this layer, so it is tested here once, against the ``Line``
contract, rather than through either reader.
"""
...
from slipguard.extractors.kie import (
    Line,
    _find_date,
    _last_money,
    _merge_rows,
...
# --- row reconstruction (some readers split a row's label & amount apart) ----
...
def test_receipt_from_lines_maps_all_fields_on_image_route():
    lines = [
    ...
    r = receipt_from_lines(
        lines, "d1", source=DocumentType.IMAGE, source_path="/img/d1.jpg", image_path="/img/d1.jpg"
    )
...
def test_receipt_from_lines_sets_pdf_provenance_without_image_path():
    # The PDF route: source must be PDF and source_path the PDF (so pdf_meta runs), and
    # image_path stays None (so image-EXIF forensics correctly abstain).
    lines = [_ln("Subtotal 5.00", y=0.80), _ln("Total 5.50", y=0.90)]
    r = receipt_from_lines(lines, "d", source=DocumentType.PDF, source_path="/x/d.pdf")
    assert r.source is DocumentType.PDF
    assert r.source_path == "/x/d.pdf"
    assert r.image_path is None


def test_receipt_from_lines_recovers_split_two_column_summary():
    # docTR/some PDFs emit each summary row's amount and label as SEPARATE lines at the
    # same height (amount in the right column, x larger). The row-merge must reunite them
    # so subtotal/tax/total are read — and the orphaned amounts must NOT survive as items.
    lines = [
    ...
    r = receipt_from_lines(lines, "d", source=DocumentType.IMAGE, source_path="/i.jpg", image_path="/i.jpg")
...
def test_receipt_from_lines_clean_read_records_no_confidence():
    ...
    # arithmetic guard behaves exactly as on the oracle path (no regression). This is also
    # the born-digital PDF case (exact text is always conf 1.0).
    lines = [
        _ln("Subtotal 5.00", y=0.80),
        _ln("Tax 0.50", y=0.85),
        _ln("Total 5.50", y=0.90),
    ]
    r = receipt_from_lines(lines, "d", source=DocumentType.PDF, source_path="/i.pdf")
    assert r.field_confidence == {}


def test_receipt_from_lines_records_low_reading_confidence():
    lines = [
