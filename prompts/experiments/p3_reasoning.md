...
## Work through it internally before answering

Reason silently in this order, then write only the JSON:
1. List every printed amount and the label next to it.
2. Assign `total` = the amount paid, `subtotal` = the pre-tax sum, `tax` = the tax line(s),
   `service_charge` / `discount` only if printed.
3. Check `subtotal + tax + service_charge − discount = total` (after allowing service/discount/
   tax-inclusive pricing) → set `arithmetic_consistent`.
4. Read and normalise the date; judge `date_valid`.
5. Note any visible edit cues; weigh the overall decision.

Do **not** output your working, scratch notes, or any prose — output **only** the JSON object below.
