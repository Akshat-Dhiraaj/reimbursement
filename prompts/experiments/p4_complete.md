...
## Extracting the amounts (do NOT leave printed values blank)

These fields drive the audit — read them carefully and fill every one you can see:

- **total** — the final amount the customer pays (`TOTAL`, `GRAND TOTAL`, `AMOUNT DUE`,
  `BALANCE DUE`), not the cash tendered or the change.
- **subtotal** — the pre-tax sum of items (`SUBTOTAL`, `SUB-TOTAL`, `NET`).
- **tax** — the total tax / VAT / GST / sales-tax amount printed (sum it if several tax lines show).

Rules:
1. **If a line is printed on the receipt, fill its field — never return `null` for a value you can
   actually see.** Tax and subtotal are usually in small text just above the total — look there
   before deciding a field is absent.
2. These three relate by `total = subtotal + tax` (plus service charge, minus discount). If
   **exactly one** of {subtotal, tax, total} is missing while the other two are clearly printed and
   there is **no** service charge or discount, **derive** the missing one (subtotal = total − tax;
   tax = total − subtotal). Otherwise leave it `null`.
3. Return numbers as plain decimals — no currency symbols, no thousands separators, a dot decimal
   point (`1,234.50` → `1234.50`).
