...
**Status (2026-06) — confirmed by survey, acted on:** a fresh web survey reconfirmed the
insight, and we built the **lightweight provenance alternative** rather than the heavy pixel
route. Shipped: PDF `/Prev` content-edit localization + signature edit-after-signing (#71/#72),
and **C2PA / Content Credentials** reading (#76) — a signed `trainedAlgorithmicMedia` assertion
is the *one* trustworthy positive for an AI-edited photo (high-precision / near-zero-recall, so
still never the gate). The heavy pixel route (#60) stays **deferred**: the survey reconfirmed
naive FFT AI-detection collapses under recompression, ELA is FP-prone, and PRNU needs a
per-camera reference we never have for inbound third-party receipts — and the strong 2025
detectors are GPU neural nets we've excluded for being heavy *and* (the user measured) inaccurate.
...
| **Groq hosted VLM** (API extraction; `llama-4-scout-17b-16e-instruct`) | ✅ **In use** (opt-in, `GROQ_API_KEY`) | The **API paradigm** for the IMAGE route — a *light client* (stdlib `urllib`, **no new dep**, no GPU) calling a *large hosted model*. Reuses the Qwen prompt + JSON→Receipt parser (DRY). **Measured macro 0.847** (N=50, 0 errors, `eval-extract --extractor groq`; a first N=12 run read an optimistic 0.947) vs the local Qwen2-VL-2B's 0.725 — a big hosted model reads markedly better. **The tradeoff (the [SCORECARD.md](SCORECARD.md) point): the image leaves the box** (egress), plus **rate limits** (free tier 429'd a rapid batch → added 429 retry-with-backoff) and a network dependency. Egress was cleared by the user for this tool, so it is kept opt-in. Implementation notes: Groq's Cloudflare 1010-blocks default urllib/datacenter UAs (we send a browser UA, harmless from residential IPs); the urllib client avoids adding the `groq`/`openai` SDK. |
| **`groq`/`openai` SDK** (for the Groq call) | ❌ Not used | Groq's API is OpenAI-compatible, so a 20-line stdlib `urllib` POST does the job with **zero new dependency** — aligns with the lightweight/pure-Python ethos and keeps the core install unchanged. We add the SDK only if we need streaming / tool-use ergonomics later. |
...
| **c2pa-python** (C2PA / Content Credentials reader for `image_meta`) | ✅ **In use** (opt-in `[c2pa]`) | **MIT OR Apache-2.0**, wrapping the Rust c2pa-rs. The one IMAGE *trustworthy positive*: a signed `trainedAlgorithmicMedia` manifest (Firefly / DALL·E / Sora / Imagen) is cryptographic evidence of AI generation/edit; a `digitalCapture` (Pixel 10 / Galaxy S25) signs a genuine capture. CPU-only and network-free for reads — but a **heavy ~260 MB install** (measured; the survey's ~90 MB estimate was low), so kept an optional extra and `image_meta` falls back to EXIF when absent. Honest: high-precision / **near-zero-recall** (sparse adoption + strippable), so it complements — never gates — the arithmetic/duplicate backstop. Detection is deterministic schema-parsing, validated by unit tests + a real-`Reader` integration test (no signed benchmark fixture — c2pa-rs's strict signing-cert profile makes minting impractical). |
| **fontTools** (PDF embedded-font / subset inspection) | 🔜 **Approved, not yet used** (for #73) | **MIT**, pure-Python, light. Pre-approved for the upcoming PDF font-anomaly signal (a pasted-in value drawn in a different / non-embedded / mismatched-subset font vs. its row); not yet wired. |
| **CLIP / ViT AI-image detectors** (e.g. C2P-CLIP, Community Forensics) | ❌ **Deferred (#60)** | Permissive, but a fresh survey reconfirmed they are **low-ROI under our constraints** — the strong 2025 detectors are GPU neural nets (heavy *and*, as the user measured, inaccurate on our data), and cheap pixel heuristics (FFT / ELA / PRNU) collapse under recompression or need a per-camera reference we never have. The **lightweight provenance alternative shipped instead** (c2pa-python, above; PDF `/Prev` + signature signals). If ever added, only a calibrated weak input per §1.2, never the gate. |
...
claim than either alone, and it named a concrete fix — a richer Receipt model — now **done
(#81): adding `service_charge` / `discount` (total reconciles as subtotal+tax+service−discount)
and accepting tax-inclusive line prices cut CORD's FP 0.170 → 0.030 (~5.7×) and nudged
WildReceipt 0.364 → 0.324**, rather than hiding the flags. (CORD also surfaced — and we fixed —
a 1000× `Rp.`-prefix money-parser bug; §2.)
